@props(['packages', 'systemPackages', 'pendingInstallations', 'userState' => 'idle', 'activePackage' => null, 'viewMode' => 'grid'])
    ...
    <div class="mb-14">
        ...
        @else
            <div class="flex flex-col gap-4">
                @foreach($systemPackages as $pkg)
                    <x-store.package-list-card :$pkg :$pendingInstallations :$userState :$activePackage wire:key="sys-{{ $pkg['name'] }}" />
                @endforeach
            </div>
        @endif
    </div>
...
@else
    <div class="flex flex-col gap-4">
        @foreach($packages as $pkg)
            <x-store.package-list-card :$pkg :$pendingInstallations :$userState :$activePackage wire:key="home-{{ $pkg['name'] }}" />
        @endforeach
    </div>
@endif
