...
>
    ...
    >
        ...
        <div class="p-8 flex gap-4 mt-auto">
            ...
            @else
                <button wire:click="install('{{ $selectedPackage['Name'] }}', {{ $selectedPackage['is_aur'] ?? false ? 'true' : 'false' }}, {{ $selectedPackage['is_flatpak'] ?? false ? 'true' : 'false' }})" class="flex-1 h-12 bg-bamboo text-white text-xs font-black rounded-md hover:bg-bamboo/90 transition-all uppercase tracking-widest shadow-lg shadow-bamboo/20">Install Now</button>
            @endif
        </div>
    </div>
